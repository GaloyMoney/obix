    ...
    Tables: MailboxTables,
{
    ...
    ///
    /// Subsequent [`new_events`](Self::new_events) / [`map_new`](Self::map_new)
    /// reads with the returned cursor see only events published (buffered on
    /// the op's commit hook) after this call — e.g. the events generated by a
    /// specific downstream call that publishes onto the same op. See
    /// [`OpCursor`] for semantics and caveats.
    ///
    ...
    /// advances the cursor.
    ///
    /// Convenience over [`new_events`](Self::new_events) for the republish
    /// use case: map this outbox's freshly published events into another
    /// outbox's payload type and publish them onto the same op —
    ...
    pub fn map_new<T>(
        ...
        self.new_events(op, cursor).iter().filter_map(f).collect()
    }
