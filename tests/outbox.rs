...
async fn delivers_events_notified_while_listen_connection_down() -> anyhow::Result<()> {
    ...
    sqlx::query(
        ...
        WITH kill AS (
            ...
            UPDATE persistent_outbox_events_batch_head
            SET last_sequence = last_sequence + 1
            WHERE singleton
            RETURNING last_sequence
        ), inserted AS (
            INSERT INTO persistent_outbox_events (sequence, payload)
            SELECT last_sequence, $1::jsonb FROM reserved, (SELECT count(*) FROM kill) _forced
            RETURNING sequence
        )
        INSERT INTO persistent_outbox_events_batches (first_sequence, last_sequence)
        SELECT sequence, sequence FROM inserted
        "#,
    )
    .bind(r#"{"Ping": 2}"#)
    .execute(&pool)
...
async fn unnotified_events_delivered_via_idle_resync() -> anyhow::Result<()> {
    ...
    helpers::write_history_direct(&pool, &[r#"{"Ping": 42}"#]).await?;
...
async fn backfill_replays_across_many_small_pages() -> anyhow::Result<()> {
    ...
    let history: Vec<String> = (0..25u64).map(|n| format!(r#"{{"Ping": {n}}}"#)).collect();
    let refs: Vec<&str> = history.iter().map(String::as_str).collect();
    helpers::write_history_direct(&pool, &refs).await?;
...
async fn slow_consumer_does_not_inflate_the_rows_read() -> anyhow::Result<()> {
    ...
    let history: Vec<String> = (0..HISTORY)
        .map(|n| format!(r#"{{"Ping": {n}}}"#))
        .collect();
    let refs: Vec<&str> = history.iter().map(String::as_str).collect();
    helpers::write_history_direct(&pool, &refs).await?;
